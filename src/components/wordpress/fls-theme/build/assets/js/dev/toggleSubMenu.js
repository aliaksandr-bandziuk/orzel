//#region src/js/custom/toggleSubMenu.js
document.addEventListener("DOMContentLoaded", () => {
	...
	menuItems.forEach((menuItem) => {
		...
		menuItem.addEventListener("mouseenter", () => {
			if (!isDesktop()) return;
			closeAllSubMenus();
			if (subList) subList.classList.add("menu__sub-list--open");
			if (arrow) arrow.classList.add("menu__arrow--rotated");
		});
		menuItem.addEventListener("mouseleave", () => {
			if (!isDesktop()) return;
			if (subList) subList.classList.remove("menu__sub-list--open");
			if (arrow) arrow.classList.remove("menu__arrow--rotated");
		});
	});
	document.addEventListener("keydown", (event) => {
		if (event.key === "Escape" && isDesktop()) closeAllSubMenus();
	});
	document.addEventListener("click", (event) => {
		if (!isDesktop()) return;
		if (!Array.from(menuItems).some((menuItem) => menuItem.contains(event.target))) closeAllSubMenus();
	});
});
//#endregion
